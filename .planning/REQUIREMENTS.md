...
- [ ] **DATA-01**: System can load and validate universe configuration supporting 100 liquid stocks with matched market and sector benchmarks.
- [ ] **DATA-02**: System provides an abstract `MarketDataProvider` interface decoupling data sources from downstream engines.
- [ ] **DATA-03**: System implements an offline-first `ParquetDataProvider` reading curated 60-day 5-minute OHLCV candle datasets with zero network dependency.

### Replay Engine & Streaming

- [ ] **REPL-01**: User can stream synchronized 5-minute candles across the 100-stock universe in sequential order.
...
- [ ] **FEAT-02**: System calculates Time-of-Day (TOD) slot-bucketed rolling baselines to eliminate market open/close diurnal U-curve false alarms.
- [ ] **FEAT-03**: System calculates sector-relative and market-relative excess returns against configured benchmarks.
...
- [ ] **FUSE-01**: System combines statistical detector signals, ML anomaly scores, and market context into a unified 0–100 risk score.
- [ ] **FUSE-02**: System applies non-linear saturation curves ensuring extreme single-metric shocks (e.g. 10x volume) produce high-risk alerts.
...
- [ ] **EXPL-01**: System generates human-readable natural language explanations identifying the top contributing features and deviations.
...
- [ ] **EXPL-03**: System appends an auditable evidence schema and mandatory regulatory surveillance disclaimer to all alert payloads.
...
### FastAPI Surveillance Service

- [ ] **API-01**: System exposes REST endpoints for system health, universe metadata, and replay control (`/replay/start`, `/replay/pause`, etc.).
- [ ] **API-02**: System exposes endpoints for active alerts, stock-specific features, and detector signals with strict Pydantic v2 schemas.
...
- [ ] **DASH-01**: User can view a real-time surveillance dashboard with active alert feed, severity badges, and replay status indicators.
...
- **NEWS-01**: System ingests free RSS/Finnhub news feeds and overlays sentiment markers on flagged stock timelines.
- **NEWS-02**: System cross-references volume spikes with breaking press releases.

## Out of Scope

| Feature | Reason |
|---------|--------|
| Stock Price Forecasting & Prediction | Surveillance detects unusual behavior; price forecasting is trading/speculation and prone to hallucination. |
| Automated Trading Bot Execution | Surveillance is decision support for human analysts; automated execution introduces out-of-scope financial risk. |
...
| Distributed Clusters (Kafka/Spark/K8s) | Massive operational overhead unnecessary for local 100-stock 5-minute replay. |
